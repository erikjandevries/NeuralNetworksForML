function a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  % fprintf('%d, %d\n', size(model.input_to_hid, 1), size(model.input_to_hid, 2));
  % fprintf('%d, %d\n', size(model.hid_to_class, 1), size(model.hid_to_class, 2));
  % fprintf('############');
  % fprintf('Number of training cases: %d', n_training_cases);
